...
/**
 * Vasalen gehen von Basis A zu Basis B ueber den TopPath.
 * @author Marius
 *
 */
public final class FractionATopPath {

	/**
	 * darf nicht instanziert werden.
	 */
	private FractionATopPath() {
	...
	private static void setVertices(final Graphomat g) {
		...
		g.addVertex(0, new Action(g) {
			 ...
			 * 
			 */
			private static final long serialVersionUID = -5035167054851032716L;
		...
		g.addVertex(2, new MovingToOnePointAction(g, Map.getMatchMap().getImportantPositions().get(
				WayPoints.TOPPATH_ENTREE_TEAM_B).get(0)));
	...
	private static void setArcs(final Graphomat g) {
		...
		g.addArc(0, 1, new Condition(g) {
			 ...
			 * 
			 */
			private static final long serialVersionUID = 3217825289987555866L;
		...
		g.addArc(
				...
				3,
				new IsInAreaCondition(Map.getMatchMap().getImportantPositions()
						.get(WayPoints.TOPPATH_ENTREE_TEAM_B_AREA), g));
