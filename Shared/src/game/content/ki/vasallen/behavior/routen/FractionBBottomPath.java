...
/**
 * Vasalen gehen von Basis B zu Basis A ueber den BottomPath.
 * @author Marius
 *
 */
public final class FractionBBottomPath {

	/**
	 * darf nicht instanziert werden.
	 */
	private FractionBBottomPath() {
	...
	private static void setVertices(final Graphomat g) {
		...
		g.addVertex(0, new Action(g) {
			 ...
			 * 
			 */
			private static final long serialVersionUID = 6528613590059495532L;
		...
		g.addVertex(2, new MovingToOnePointAction(g, Map.getMatchMap().getImportantPositions().get(
				WayPoints.BOTTOMPATH_ENTREE_TEAM_A).get(0)));
	...
	private static void setArcs(final Graphomat g) {
		...
		g.addArc(0, 1, new Condition(g) {
			 ...
			 * 
			 */
			private static final long serialVersionUID = 9002008688994318606L;
		...
		g.addArc(
				...
				3,
				new IsInAreaCondition(Map.getMatchMap().getImportantPositions()
						.get(WayPoints.BOTTOMPATH_ENTREE_TEAM_A_AREA), g));
