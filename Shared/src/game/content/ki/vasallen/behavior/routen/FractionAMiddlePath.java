...
/**
 * Vasalen gehen von Basis A zu Basis B ueber den MiddlePath.
 * @author Marius
 *
 */
public final class FractionAMiddlePath {

	/**
	 * darf nicht instanziert werden.
	 */
	private FractionAMiddlePath() {
	...
	private static void setVertices(final Graphomat g) {
		...
		g.addVertex(0, new Action(g) {
			 ...
			 * 
			 */
			private static final long serialVersionUID = 7427186384473053643L;
		...
		g.addVertex(2, new MovingToOnePointAction(g, Map.getMatchMap().getImportantPositions().get(
				WayPoints.MIDDLEPATH_ENTREE_TEAM_B).get(0)));
	...
	private static void setArcs(final Graphomat g) {
		...
		g.addArc(0, 1, new Condition(g) {
			 ...
			 * 
			 */
			private static final long serialVersionUID = 715407803285573383L;
		...
		g.addArc(
				...
				3,
				new IsInAreaCondition(Map.getMatchMap().getImportantPositions()
						.get(WayPoints.MIDDLEPATH_ENTREE_TEAM_B_AREA), g));
