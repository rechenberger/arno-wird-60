...
/**
 * Zeichnet ein Modal. Hintergrund wird verdunkelt und man kann erst weiter
 * machen, bis man das Modal beantwortet hat.
 * 
 * @author Sean
 */
public class Modal extends JPanel {
	 ...
	 */
	protected int boxHeight = 160;

	/**
	 * Mindestbreite des Headers.
	 */
	protected int headerWidth = 350;

	/**
	 * Aeusserer Rand der Box.
	 */
	protected int boxMargin = 30;

	/**
	 * Innerer Rand der Box.
	 */
	protected int boxPadding = 20;

	/**
	 * Aeusserer Rand des Headers.
	 */
	protected int headerMargin = 8;

	/**
	 * Innerer Rand des Headers.
	 */
	protected int headerPadding = 10;

	/**
	 * Groesse der ersten, schraegen Ecke.
	 */
	protected int boxEdge1 = 10;

	/**
	 * Groesse der zweiten, schraegen Ecke.
	 */
	protected int boxEdge2 = 15;

	/**
	 * Luecke zwischen Header und Box.
	 */
	protected int boxHeaderGap = 10;
	 ...
	 *            Titel des Modals
	 * @param message
	 *            Nachricht des Modals
	 */
	public Modal(final String head, final String message) {
		this.head = head;
	 ...
	 *            Label des Buttons
	 * @param clicked
	 *            MouseListener, der dem Button hinzugefuegt wird. null, falls
	 *            keiner eingebaut werden soll
	 */
	...
	/**
	 * Zeichnet den Header/Titel ausgehend von der uebergebenen Breite/Hoehe und
	 * dem Startpunkt, der in der Methode drawBox gesetzt wird. Deshalb muss
	 * drawBox vor drawHeader aufgerufen werden.
	 * 
	 * @param startWidth
	 *            Breite des Headers
	 * @param startHeight
	...
	protected void drawHeader(final double startWidth, final double startHeight) {
		...
		double height = startHeight + this.headerMargin;
		...
		// Abstand von der Box
		double y = this.startBox.y - height - this.boxHeaderGap;
		...
		StringHelper.drawString(g2, this.head,
				(int) (this.startHeader.x + this.headerPadding),
				(int) (this.startHeader.y + 2));
	}
	...
	protected void drawBox(final double startWidth, final double startHeight) {
		...
		// Groesse wird um die Aussenraender 2 * 30 erweitert, damit man ein
		// wenig
		// mehr Platz hat und es huebscher ist.
		width += 2 * this.boxMargin;
		height += 2 * this.boxMargin;
		...
		StringHelper.drawString(g2, this.message,
				(int) (this.startBox.x + this.boxPadding),
				(int) (this.startBox.y + this.boxPadding));
	}
	...
	/**
	 * Zeichnet die Box und den Header.
	 */
	protected void drawBoxAndHeader() {
		Rectangle2D rect;

		rect = StringHelper.stringSize(g2, this.message);
		this.drawBox(rect.getWidth(), rect.getHeight());

		rect = StringHelper.stringSize(g2, this.head);
		this.drawHeader(rect.getWidth(), rect.getHeight());
	}
	...
	/**
	 * Zentrale Methode zum andere Zeichenmethoden aufzurufen.
	 */
	protected void draw() {
		this.center = new Point(this.getWidth() / 2, this.getHeight() / 2);
		this.drawBackground();
		this.drawBoxAndHeader();
		this.drawButtons();
	}
